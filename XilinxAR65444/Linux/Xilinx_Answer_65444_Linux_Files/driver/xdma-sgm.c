...
// SANGJIN
int sgm_get_multi_user_pages(struct sg_mapping_t *sgm, struct xdma_multiple_sgdma_ioctl *args, size_t count, int to_user)
{
	//u64 boe = (u64)start;
	...
	const int nr_pages = (count >> PAGE_SHIFT);
    ...
    for (i = 0; i < args->va_cnt; i++) { 
	    rc += get_user_pages_fast((unsigned long)args->va_arr[i], args->size_arr[i] >> PAGE_SHIFT, to_user/*write*/, &pages[j]);
	    //pr_debug("get_user_pages_fast(%lu, nr_pages = %d) == %d.\n", (unsigned long)start, nr_pages, rc);
        j += args->size_arr[i] >> PAGE_SHIFT;
    }
	//rc = get_user_pages_fast((unsigned long)args->va_arr[0], nr_pages, to_user/*write*/, pages);
	...
	sg_set_page(&sgl[0], pages[0], 0 /*length*/, offset_in_page(args->va_arr[0]));
