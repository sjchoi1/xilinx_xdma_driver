...
// SANGJIN
static struct xdma_transfer *transfer_multi_create(struct xdma_dev *lro,
		struct xdma_multiple_sgdma_ioctl *args, size_t cnt, u64 ep_addr, int dir_to_dev,
		int non_incr_addr, int force_new_desc, int userspace)
{
	int i = 0;
	int last = 0;
	...
	/* lock user pages in memory and create a scatter gather list */
    rc = sgm_get_multi_user_pages(transfer->sgm, args, cnt, !dir_to_dev);
    //rc = sgm_get_user_pages(transfer->sgm, args->va_arr[0], args->size_arr[0], !dir_to_dev);
...
// SANGJIN
static ssize_t transfer_multi_data(struct xdma_engine *engine, 
                                   ...
                                   loff_t *pos, int seq)
{
    ...
    for (i = 0; i < args->va_cnt; i++) 
        remaining += args->size_arr[i];
	...
	while ((res == 0) && (remaining > 0)) {
		...
		/* build device-specific descriptor tables */
		transfer = transfer_multi_create(lro, args, transfer_len,
			      *pos, engine->dir_to_dev, engine->non_incr_addr, 0, 1);
        /*transfer = transfer_create(lro, args->va_arr[0], args->size_arr[0],
                *pos, engine->dir_to_dev, engine->non_incr_addr, 0, 1);
        */
		dbg_tfr("seq:%d transfer=0x%p.\n", seq, transfer);
...
// SANGJIN 
static int ioctl_do_multiple_write(struct file *file, unsigned long arg)
{
    int i, rc;
    ssize_t res = 0;
    static int counter;
    ...
    loff_t *pos = &pos_val;
    //res = char_sgdma_read_write(file, (char *)args->va_arr[0], args->size_arr[0], &pos, 1);
    ...
    res = transfer_multi_data(engine, args, pos, seq); 
	dbg_tfr("seq:%d ioctl_multiple_write() return=%lld.\n", seq, (s64)res);
		...
		unsigned long arg)
{
    ...
    // SANGJIN
    case IOCTL_XDMA_MULTIPLE_WRITE:
